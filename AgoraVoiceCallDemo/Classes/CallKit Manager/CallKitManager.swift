...
extension CallKitManager {
    ...
    func endCall(uuid: UUID) {
        ...
        NC.post(name: NSNotification.Name(rawValue: Constants.NotifId.LEAVE_CHANNEL), object: nil)
    ...
    fileprivate func navigateToVoiceCallScreen() {
        ...
        guard APPDELEOBJ.channelId.count > 0 else {
            APPDELEOBJ.window?.rootViewController?.showAlertWithDismiss(title: "", message: "Please enter channel id") { }
            return
        }
        
        let voiceCallVC = GetStoryBoard(identifier: "VoiceCallVC", storyBoardName: Constants.Storyboard.Main) as! VoiceCallVC
        voiceCallVC.channelId = APPDELEOBJ.channelId
        voiceCallVC.uuid = APPDELEOBJ.uuid
        voiceCallVC.isCalling = false
        APPDELEOBJ.navController?.pushViewController(voiceCallVC, animated: true)
    }
